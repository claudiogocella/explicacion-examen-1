...
//                           0        1        2        3        4
// ventas [0][1] : ventas de la 2 semana del vendedor llamado Camila (NO SE PUEDE INVERTOR EL ORDEN, SINO DA INFO ERRÓNEA)
//ventas [1][0] : ventas de la 1 semana del vendedor llamado Sofìa
...
function mostrarVenta(maxMin: number) { //maxMin = 1
  for (let i: number = 0; i < ventas.length; i++) {
    ...
    semana++; //el ++ incrementa 1 semana cada vuelta
    let msg: string = "La venta ";
    msg += maxMin === 1 ? "máxima" : "mínima";
    msg += " de ";
    msg += nombres[i];
    msg += " es de $";
    msg += ventas[i][indice]; // este indice lo saca de la linea 75 - ademas, ese indice vino como retorno de la posocion fila 90
